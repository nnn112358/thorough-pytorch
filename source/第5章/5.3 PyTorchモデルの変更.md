...
学習時に最終出力だけでなく、中間層の出力にも監督を与えたい場合があります。その場合は `forward` の戻り値に中間出力を追加します。

例として resnet50 の 10 クラス分類で、最終手前の 1000 次元と最終 10 次元の両方を返す実装を示します：
