...
深層学習ではネットワークパラメータを更新して損失を下げます。ResNet-50 のように数千万規模の係数を持つモデルでは、勾配に基づく最適化が現実的な方法です。

1. 第一种是直接暴力穷举一遍参数，这种方法从理论上行得通，但是实施上可能性基本为0，因为参数量过于庞大。
2. 为了使求解参数过程更快，人们提出了第二种办法，即BP+优化器逼近求解。

最適化器は逆伝播で得た勾配を用いてパラメータを更新し、損失を下げて予測を真値に近づけます。
...
PyTorch は `torch.optim` に多数の最適化器を提供しています。
...
これらはすべて `Optimizer` を継承します。基底クラス `Optimizer` は次の通り：
...
+ `defaults`: 最適化器のハイパーパラメータ（例）

```python
{'lr': 0.1, 'momentum': 0.9, 'dampening': 0, 'weight_decay': 0, 'nesterov': False}
```

+ `state`: パラメータごとの内部状態（例）
