...
深層学習ではモデルが学習（訓練）を通じて良い性能に近づきます。そのための“負のフィードバック”が損失関数です。

![](./figures/3.5.1lossfunciton.png)

損失はモデル出力と真値のズレを定量化する指標で、これを最小化するようにパラメータを更新します。

以下に PyTorch（`torch.nn`）でよく使う損失関数を示し、用途・数式・使い方を簡潔にまとめます。
...
用途：二値分類における交差エントロピー。入力は確率（sigmoid 後など）。ラベルは {0,1}。
...
主な引数：`weight`、`ignore_index`、`reduction` など。
...
用途：`y` と `target` の要素ごとの絶対誤差。

`reduction` は `'none'|'sum'|'mean'` を選択。`'none'` は要素ごと。
