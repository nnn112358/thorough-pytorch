...
Residual Network（ResNet）は Microsoft Research の He らが提案したアーキテクチャです。層を深くすると学習精度が頭打ちになり、さらに深くすると逆に悪化する「劣化現象」を指摘し、入力を後段に素通しするショートカット接続でこれを緩和しました。これにより 100 層以上の非常に深いネットワークの学習が現実的になりました。

本稿のゴール：

- 勾配消失/爆発の概念
- BasicBlock と Bottleneck を分ける理由
- `expansion` の役割

## 1 基本紹介

多層化は性能向上に寄与しますが、深くしすぎると勾配消失/爆発・情報伝達の損失などで学習が困難になります。ResNet はショートカット接続で勾配を直接流し、深いネットの学習を安定化しました。以下で勾配消失・勾配爆発の概要を確認します。

勾配爆発/消失は、深いネットの構造と逆伝播の性質に起因します。損失から計算される誤差勾配は重み更新の方向と大きさを与えますが、深層/再帰では勾配が累積して極端に大きくなり（オーバーフローや NaN、学習不安定化）、あるいは極端に小さくなって（前段に伝わらず）更新が止まることがあります。層間で 1.0 より大きい値を繰り返し乗じると指数的増大（爆発）、1.0 未満だと指数的減衰（消失）が起きます。例えば tanh のような活性では前段に行くほど勾配が指数的に小さくなり、学習が停滞します。
## 2 源码解读

以下では [torchvision の ResNet 実装](https://github.com/pytorch/vision/blob/master/torchvision/models/resnet.py) を手掛かりに構造を読み解きます。

### 2.1 畳み込みのラッパ

はじめに 3×3/1×1 の畳み込みを関数化して可読性を上げています。畳み込み・活性化・BN をまとめるパターンも一般的です。
